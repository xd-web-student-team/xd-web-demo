...
const store =  new Vuex.Store({
  state:sessionStorage.getItem('state') ? JSON.parse(sessionStorage.getItem('state')) :{
    routes:[],
    sessions: { '群聊': {0:{ groupMsgContentList:[],idGroup:1}}},//聊天记录
    users: [],//用户列表
    groups: [],//群列表
    currentUser: null,//当前登录用户
    currentSession: { username: '西电人总群', nickname: '西电人总群' },//当前选中的用户，默认为群聊
    currentSessionGroup: null,
    currentList:'西电人总群',//当前聊天窗口列表
    filterKey:'',
    stomp:null,
    isDot:{},//两用户之间是否有未读信息
  ...
  mutations:{
    ...
    //请求并保存所有加入的群组
    GET_GROUPS(state){
      getRequest("/chat/groups").then(resp=>{
        if (resp){
          state.groups = resp;
        }
      })
