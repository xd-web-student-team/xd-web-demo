  ...
  <div id="toolbar">
      ...
      <div class="bottomBtnBar">
        <el-tooltip class="item" effect="dark" content="添加..." placement="right">
          ...
          <el-popover placement="right" width="180" trigger="click" popper-class="moreListPopoverClass">
            ...
            <el-button slot="reference" class="toolBtn" size="small"><i class="fa fa-plus fa-2x" aria-hidden="true"></i></el-button>
          </el-popover>
        </el-tooltip>
    ...
    <el-dialog title="新建群聊" :before-close="closeBuildGroupDialog" :visible.sync="buildGroupDialog" width="30%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="info" @click="buildGroup('groupForm')" style="width: 100%">创建群聊</el-button>
      </div>
    </el-dialog>
    <el-dialog title="加入群聊" :before-close="closeBuildGroupDialog" :visible.sync="joinGroupDialog" width="30%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="info" @click="joinGroup('groupForm')" style="width: 100%">加入群聊</el-button>
      </div>
    </el-dialog>
      ...
      var validateJoinGroup = (rule, value, callback) => {
        ...
        this.getRequest("chat/checkGroupName?groupName=" + value).then((resp) => {
          if (resp == 0) {
            ...
            this.getRequest("chat/checkJoined?idGroup=" + resp.id).then((resp2) => {
              if(resp2 == 0) {
                callback()
              }
              else {
        ...
        this.$refs.groupForm.validate((valid) => {
          if (valid) {
            this.postRequest("/chat/buildGroup", this.groupForm).then((resp) => {
              Vue.set(this.$store.state.sessions['群聊'], resp.id, {groupMsgContentList:[], idGroup:resp.id});
